...
for(j in 2:37){
  ...
  for(day in pp.main.year2){
    pp.year.text <- html_text(day)
  ...
  a <- c('http://www.ppomppu.co.kr/zboard/zboard.php?id=car_consult','http://www.ppomppu.co.kr/zboard/zboard.php?id=car_service')
  if(pp.main.attr != a){
    for(i in pp.main.attr){
      if(i != '#'){
...
getwd()
write.csv(years.df, '뽐뿌날짜.csv')
write.csv(title.df, '뽐뿌제목.csv')
write.csv(contents.df, '뽐뿌본문.csv')
write.csv(reply.df, '뽐뿌댓글.csv')
